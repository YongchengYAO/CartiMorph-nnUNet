...
from CartiMorph_nnUNet.evaluation.add_mean_dice_to_json import foreground_mean
from CartiMorph_nnUNet.evaluation.evaluator import evaluate_folder
from CartiMorph_nnUNet.evaluation.model_selection.ensemble import ensemble
from CartiMorph_nnUNet.paths import network_training_output_dir
import numpy as np
from subprocess import call
from CartiMorph_nnUNet.postprocessing.consolidate_postprocessing import consolidate_folds, collect_cv_niftis
from CartiMorph_nnUNet.utilities.folder_names import get_output_folder_name
from CartiMorph_nnUNet.paths import default_cascade_trainer, default_trainer, default_plans_identifier
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += "CartiMorph_nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + m1 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + " -z\n"
                    predict_str += "CartiMorph_nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr " + tr + " -ctr " + trc + " -m " + m2 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + " -z\n"

                    if not disable_postprocessing:
                        predict_str += "CartiMorph_nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp " + join(network_training_output_dir, "ensembles", id_task_mapping[t], k, "postprocessing.json") + "\n"
                    else:
                        predict_str += "CartiMorph_nnUNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER\n"
                else:
                    predict_str += "CartiMorph_nnUNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + k + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                print(predict_str)
